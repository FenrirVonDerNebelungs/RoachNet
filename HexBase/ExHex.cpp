...
unsigned char ExHex::spawn(s_HexPlate* plate) {
	if (plate == NULL)
		return ECODE_ABORT;
...
unsigned char ExHex::copyBasePlateWeave(/*const*/ s_HexPlate* base, s_HexPlate* exPlate) {
	...
	for (long ii = 0; ii < base->N; ii++) {
		...
		for(int i_web =0; i_web<6; i_web++){
			...
			if (linkedBaseHex != NULL) {
				...
				s_Hex* linkedExHex = exPlate->get(linkedBaseIndex);
				exHex->web[i_web] = (s_Node*)linkedExHex;
				int rev_web_i = n_HexRoot::revWeb_i(i_web);
...
unsigned char ExHex::weaveExHexes(s_HexPlate* plate) {
	if (plate == NULL)
		return ECODE_ABORT;
	...
	for (long base_hex_i = 0; base_hex_i < numBaseHexes; base_hex_i++) {
		s_Hex* baseHex = plate->get(base_hex_i);

		for(int i_web=0; i_web<6; i_web++) {
			s_Hex* linkedHex = baseHex->getWeb(i_web);
			if (linkedHex == NULL) {
				/*find which neighbor is linked*/
				s_Hex* nebHex_1 = baseHex->getWeb(n_HexRoot::Web_i(i_web - 1));
				s_Hex* nebHex1 = baseHex->getWeb(n_HexRoot::Web_i(i_web + 1));
				int i_web_posPre = i_web;
				s_Hex* candidateDupHex = NULL;
				if (nebHex_1 != NULL) {
					i_web_posPre += 1;
					candidateDupHex = nebHex_1->getWeb(n_HexRoot::Web_i(i_web_posPre));
					n_HexRoot::weaveSharedHexToNeighbors(candidateDupHex, nebHex_1, baseHex);
				}
				else if (nebHex1 != NULL) {
					i_web_posPre -= 1;
					candidateDupHex = nebHex1->getWeb(n_HexRoot::Web_i(i_web_posPre));
					n_HexRoot::weaveSharedHexToNeighbors(candidateDupHex, nebHex1, baseHex);
				}
				/*this is an unlinked edge hex, link to the ex hext*/
				if (candidateDupHex==NULL) {
					s_Hex* exHex = plate->get(ex_hex_i);
					n_HexRoot::stichHexes(baseHex, exHex, i_web);
					ex_hex_i++;
				}
			}
		}
...
unsigned char ExHex::weaveRoundExHexes(s_HexPlate* plate) {
	if (plate == NULL)
		return ECODE_ABORT;
	/*weave the ex hexes around the base hexes*/
	long numBaseHexes = m_numNodes - m_numExHexes;
	for(long ex_hex_i = numBaseHexes; ex_hex_i < m_numNodes; ex_hex_i++) {
		...
		for(int i_web=0; i_web<6; i_web++) {
			...
			if (linkedHex == NULL && swept_into_interior) {
				s_Hex* commonHex = exHex->getWeb(n_HexRoot::Web_i(i_web - 1));
				s_Hex* nebHex = exHex->getWeb(n_HexRoot::Web_i(i_web + 1));
				n_HexRoot::weaveSharedHexToNeighbors(commonHex, exHex, nebHex);
			}
		}
	}
	return ECODE_OK;
